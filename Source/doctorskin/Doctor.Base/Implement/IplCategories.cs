...
namespace Doctors.Base.Implement
{
    public class IplCategories: Repository<Category>, ICategories
    {
        ...
        public IplCategories(DoctorskinCoreContext dbContext, IConfiguration configuration) : base(dbContext)
        {
        ...
        public List<Category> GetCategoriesListAll(string search, int offset, int limit)
        {
            ...
            try
            {
                using (var u = unitOfWork.Create(false))
                {
                    ...
                    p.Add("@offset", offset);
                    p.Add("@limit", limit);
                    list = u.GetIEnumerable<Category>("Sp_GetCategoryListAllPaging", p).ToList();
                }
            }
            catch (Exception ex)
            {
